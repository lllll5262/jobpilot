...
先启动 JobPilot 及 MySQL、MongoDB、Milvus，再执行：

```powershell
python "evals/evaluate_resume_rag.py" --user-id 1 --provider qwen
```
...
- `answer_correctness`：回答与人工标准答案的事实及语义一致性。

逐条结果和均值保存在 `evals/results/`。应使用同一评估集比较检索参数、Prompt
